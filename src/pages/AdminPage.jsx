        ...
        <div className="max-w-7xl mx-auto px-4 pb-8">
          ...
          {tab==="dashboard"&&stats&&!loading&&(<div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              {[{icon:Users,label:"Toplam Kullanici",val:stats.totalUsers,c:"text-[#00a884] bg-[#00a884]/10"},{icon:Activity,label:"Cevrimici",val:stats.onlineUsers,c:"text-emerald-400 bg-emerald-400/10"},{icon:MessageSquare,label:"Toplam Mesaj",val:stats.totalMessages,c:"text-sky-400 bg-sky-400/10"},{icon:TrendingUp,label:"Bugun",val:stats.todayMessages,c:"text-amber-400 bg-amber-400/10"}].map((s,i)=>(<div key={i} className="bg-[#1f2c33] rounded-xl p-5 border border-[#2a3942]"><div className={`p-2.5 rounded-lg w-fit mb-3 ${s.c}`}><s.icon size={20}/></div><p className="text-2xl font-bold">{s.val??"-"}</p><p className="text-sm text-[#8696a0] mt-0.5">{s.label}</p></div>))}
            </div>
